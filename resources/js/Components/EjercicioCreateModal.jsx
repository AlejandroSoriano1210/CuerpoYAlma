...
export default function EjercicioCreateModal({ isOpen, onClose, onSuccess }) {
    ...
    const [touched, setTouched] = useState({
        nombre: false,
        musculo_objetivo: false,
    });

    const nombreError = validarRequerido(data.nombre, 'Nombre');
    const musculoObjetivoError = validarRequerido(data.musculo_objetivo, 'Músculo objetivo');

    const handleClose = () => {
        reset();
        setTouched({ nombre: false, musculo_objetivo: false });
        onClose();
    };
    ...
    const handleSubmit = (e) => {
        ...
        // Validar músculo objetivo
        if (musculoObjetivoError !== true) {
            setTouched(prev => ({ ...prev, musculo_objetivo: true }));
            return;
        }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-2xl font-bold text-gray-900">Crear Nuevo Ejercicio</h2>
                    <button
                        onClick={handleClose}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onBlur={() => setTouched({ ...touched, musculo_objetivo: true })}
                                className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 ${
                                    touched.musculo_objetivo && musculoObjetivoError !== true
                                        ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                        : 'border-gray-300'
                                }`}
                            ...
                            />
                            {touched.musculo_objetivo && musculoObjetivoError !== true && (
                                <p className="text-red-500 text-sm mt-1">{musculoObjetivoError}</p>
                            )}
                        </div>
                        ...
                        <div className="flex gap-4 justify-end pt-4 border-t border-gray-200">
                            ...
                            <button
                                type="submit"
                                disabled={processing || nombreError !== true || musculoObjetivoError !== true}
