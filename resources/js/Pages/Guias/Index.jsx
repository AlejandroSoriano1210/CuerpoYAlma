...
import { BookOpen, Check, PenBox, Trash2 } from 'lucide-react';
...
export default function GuiasIndex() {
    ...
    const handleDeleteGuia = async (guiaId, guiaTitulo) => {
        const accepted = await confirmAction(`¿Estás seguro de que deseas eliminar la guía "${guiaTitulo}"? Esta acción no se puede deshacer.`);
        if (!accepted) return;
                                        ...
                                        {guias.data.map((guia) => (
                                            <div key={guia.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition border border-gray-100 p-6 flex flex-col">
                                                ...
                                                <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
                                                    ...
                                                    {hasAnyRole(['entrenador', 'jefe_entrenadores', 'superusuario']) && (
                                                        <>
                                                            <Link href={route('guias.edit', guia.id)} className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-1 px-2 rounded text-center text-sm">
